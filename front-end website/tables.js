...
function generateHtmlTable(courseData) {
    document.getElementsByClassName('vertical-table1')[0].innerHTML = '';
    const programs = new Set();
    const quarters = new Set();
    const tableData = {};
    // Collect all unique programs and quarters
    for (const courseId in courseData) {
        const courseInfo = courseData[courseId];
        for (const [program, quarter] of Object.entries(courseInfo.program_quarter)) {
            programs.add(program);
            if (!tableData[quarter]) {
                tableData[quarter] = {};
            }
            if (!tableData[quarter][program]) {
                tableData[quarter][program] = [];
            }
            tableData[quarter][program].push([courseId, courseInfo.className]);
            quarters.add(quarter);
        }
    }

    // Convert sets to arrays and sort quarters
    programList = Array.from(programs);
    quarterList = Array.from(quarters).sort((a, b) => a - b);
    console.log(quarterList);

    let html = '';
    html += '<tr><th>Quarter</th>';
    ...
    // Rows with course information
    quarterList.forEach(quarter => {
        if (quarter == 0 || quarter == 6.5) {
            html += `<tr><th>Prerequisites</th>`;
        } else {
            html += `<tr><th>${quarter}</th>`;
        }
        programList.forEach(program => {
            //const courses = tableData[quarter][program] ? tableData[quarter][program].join(', ') : '';
            if (tableData[quarter][program]) {
                html += `<td>`;
                tableData[quarter][program].forEach(course => {
                    html += `<div id=${course[0]}>${course[1]}</div>`;
                });
                html += `</td>`;
            } else {
                html += `<td></td>`
    ...
    document.getElementsByClassName('vertical-table1')[0].innerHTML = html;
}
...
function addCourse() {
    ...
    form.setAttribute("id", "courseForm");

    const fields = [
        { label: "Course Name:", name: "course_name", required: true },
        { label: "Course Credits:", name: "course_credits", required: true },
        { label: "Course Code:", name: "course_code", required: true },
        { label: "Quarter No:", name: "quarter_no", required: false }
    ];
    ...
    select.appendChild(document.createElement("option")); // empty option
    programList.forEach(program => {
        const optionElement = document.createElement("option");
        optionElement.textContent = program;
        optionElement.setAttribute("value", program);
    ...
    form.addEventListener("submit", async event => {
        ...
        input.action = 'AddCourse';

        try {
            const response = await fetch('https://dnihzli5l8.execute-api.us-west-1.amazonaws.com/default/Action', {
                method: 'POST',
            ...
            if (response.ok) {
                ...
                if (response.status === 409) {
                    result.innerText = 'Duplicate entry in the database. Please try again.';
                } else if (response.status === 401) {
                    result.innerText = 'Unauthorized. Login first.';
                } else {
